function [n_spikes] = a_make_regressor_matrix(SUBJNAME,pup_type,session)
...
models={'m2_bin', 'm1_bin', '0bin', 'p1_bin', 'p2_bin'};

for mod = 1:numel(models)
    %Pupil path
    pup_dir=fullfile('C:\Users\lloydb\surfdrive\ExperimentData\NYU_RS_LC\data\', ...
                SUBJNAME, ['logfiles-' padi.sessions{session}], 'processed\smoothed\4_raw_pupil_mods\');
...
%     
    ...
    remove_dir1=fullfile(inputim.path, 'log', savedir, 'LC_conv');
    remove_dir2=fullfile(inputim.path, 'log', savedir, 'raw_pupil');
    remove_dir3=fullfile(inputim.path, 'log', savedir, 'canonical_conv');
    ...
    if exist(remove_dir3, 'dir')
        rmdir(remove_dir3, 's');
    end
    
    %mkdir
    saveRfiledir = fullfile(inputim.path, 'log', savedir, 'raw_pupil_models');
