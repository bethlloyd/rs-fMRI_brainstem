function [n_spikes] = a_addup_to_regs_LChrf_33plusSpike_Regs(SUBJNAME,pup_type,session)
...
pup_dir=fullfile('C:\Users\lloydb\surfdrive\ExperimentData\NYU_RS_LC\data\', ...
            SUBJNAME, ['logfiles-' padi.sessions{session}], 'processed\smoothed\3_convolved\HRF_LC');
disp(pup_dir)

if pup_type==1 % Load in pupil regressor # 2 - Diameter
    % get correct folder
    pup_filename='pupil_dilation_LC.txt';
...
elseif pup_type==2 % Load in pupil regressor # 2 - Derivative
    % get correct folder
    deriv_pup_filename='pupil_derivative_LC.txt';
...
R=[pup_int R sigextr spikereg_matrix];   %check order of regressors

%mkdir
saveRfiledir = fullfile(inputim.path, 'log', savedir, 'LC_conv');
if ~exist(saveRfiledir, 'dir')
    mkdir(saveRfiledir)
end

save(fullfile(saveRfiledir, 'firstlevel_final_regressors_33Regs_spikeRegs_LChrf.mat'),'R');
