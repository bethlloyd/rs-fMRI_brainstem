...
subjexcl_d1=[4, 15, 21, 37, 47, 57, 66, 69];   % subjs to excl from analysis day1 (note they are linked to order in list)38,62,
...
for mod = 9:10   %% NEED TO RUN SUBJ1 CONCAT AGAIN!!!!!!!!
        
    for day = 2
        %do not run excluded subjs
        if day==1 
            subjincl=subjexcl_d2;
            %disp(numel(subjincl));
        elseif day==2
            subjincl=subjexcl_d1;
            %disp(numel(subjincl));
        end
    ...
    %for pup = 1:numel(pup_type)
        ...
        % Loop over subjects
        for c_subj = subjincl  % run on subjs to concat across sessions
            %subjincl
            disp(['running..' subjlist(c_subj).name]);
            a_fcwml_mri_firstlevel(subjlist(c_subj).name, day, models{mod}, 'rsHRF_ROI_models');
