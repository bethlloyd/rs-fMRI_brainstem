function a_extract_rsHRF(BF, numBF)
...
%% Define ROIs
roi = {'DR_roi', 'MR_roi', 'VTA_roi', 'ACC_roi', 'LC_roi', 'SN_roi', 'OCC_roi', 'BF_sept_roi', 'BF_subl_roi'};
    ...
    for c_roi = 1:numel(roi)
        ...
        if numBF == 1
            ...
            elseif c_roi == 9
                ...
                if c_roi == 5 %lc
                    % save headers = event times 
                    LC_event_time(1,1)=cellstr('subj');
                    LC_event_time(1,2)=cellstr('LC');

                    % get the event times for each ROI 
                    LC_event_onsets=hrf_vec.event_bold{c_roi};
                end
                if c_roi == 3 %VTA
                    % save headers = event times 
                    VTA_event_time(1,1)=cellstr('subj');
                    VTA_event_time(1,2)=cellstr('VTA')
                    
                    % get the event times for each ROI 
                    VTA_event_onsets=hrf_vec.event_bold{c_roi};
                end
            end
...
%         
        ...
        if c_roi == 5
            LC_event_time(2:1+numel(LC_event_onsets),1)=cellstr(strcat(num2str(subjlist(c_subj).name)));
            LC_event_time(2:1+numel(LC_event_onsets),2)=num2cell(LC_event_onsets);
            
            filename=strcat([subjlist(c_subj).name, '_LC_rsHRF_event_onsets_both_days.csv']);
            savefilename=fullfile(stats, 'rsHRF', 'groupstats', BF, 'event_onsets', filename);
            cell2csv(savefilename,LC_event_time);
            clear LC_event_time
        elseif c_roi == 3
            VTA_event_time(2:1+numel(VTA_event_onsets),1)=cellstr(strcat(num2str(subjlist(c_subj).name)));
            VTA_event_time(2:1+numel(VTA_event_onsets),2)=num2cell(VTA_event_onsets);
        
            
            filename=strcat([subjlist(c_subj).name, '_VTA_rsHRF_event_onsets_both_days.csv']);
            savefilename=fullfile(stats, 'rsHRF', 'groupstats', BF, 'event_onsets', filename);
            cell2csv(savefilename,VTA_event_time);
            clear VTA_event_time
        end
